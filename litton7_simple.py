    ...
    def __init__(self, model_path=None, device="auto"):
        ...
        if model_path is None:
            ...
            if not Path(model_path).exists():
                model_url = f"https://lclab.thu.edu.tw/modelzoo/{model_path}"
                with urlopen(model_url) as resp, open(model_path, "wb") as fmodel:
...
def process_images(
    input_paths, output_path="results.csv", model_path=None, device="auto"
):
    """處理多張圖片並將結果寫入CSV"""
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-d", "--device", default="auto", help="運算裝置 (auto/cpu/cuda)"
    )
